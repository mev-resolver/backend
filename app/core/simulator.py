...
class AttackSimulator:
    """
    Submits REAL sandwich attack transactions to Sepolia testnet.
    Requires configured private keys and deployed contracts.
    Every attack costs real testnet gas.
    """
    ...
    async def _execute_real_sandwich(self) -> dict:
        ...
        t_start = time.time()
        logger.info("=== Real sandwich attack starting on Sepolia ===")
        ...
        buy_gas = getattr(s_buy, "gasPrice", getattr(s_buy, "maxFeePerGas", 0))
        buy_payload = {
            "tx_hash": buy_hash, "from": bot1.address,
        ...
        # Victim swap (held for MEV protection)
        s_victim = await build_and_sign_swap(
            w3, victim, res_addr, olv_addr, AMT_VIC, 0, nonce_victim, "medium")
        vic_hash = "0x" + w3.keccak(s_victim.raw_transaction).hex()
        nonce_victim += 1
        logger.info("VICTIM TX held for protection: %s", vic_hash)

        vic_gas = getattr(s_victim, "gasPrice", getattr(s_victim, "maxFeePerGas", 0))
        vic_payload = {
            "tx_hash": vic_hash, "from": victim.address,
            "token_in": "RES", "token_out": "OLV",
        ...
        # Bot2 SELL approval + tx (low gas - back-run)
        s = await build_and_sign_approve(w3, bot2, olv_addr, dex_addr, AMT_BOT, nonce_bot2)
        await send_raw_tx(w3, s); nonce_bot2 += 1

        s_sell = await build_and_sign_swap(
            w3, bot2, olv_addr, res_addr, AMT_BOT, 0, nonce_bot2, "low")
        sell_hash = await send_raw_tx(w3, s_sell); nonce_bot2 += 1
        logger.info("BOT SELL: %s", sell_hash)

        sell_gas = getattr(s_sell, "gasPrice", getattr(s_sell, "maxFeePerGas", 0))
        sell_payload = {
            "tx_hash": sell_hash, "from": bot2.address,
            "token_in": "OLV", "token_out": "RES",
            "amount_in": float(Web3.from_wei(AMT_BOT, "ether")),
            "gas_price": sell_gas, "is_bot": True, "color": "orange",
            "etherscan_url": EXPLORER + sell_hash,
        }
        store_event("transaction_arrived", sell_payload)
        store_transaction({**sell_payload, "status": "submitted", "attack_id": None,
                           "bundle_id": None, "created_at": time.time()})
        await manager.broadcast({"type": "transaction_arrived", "data": sell_payload, "timestamp": time.time()})

        # Register raw victim tx for Flashbots bundle
        mitigation_engine.store_pending_raw(vic_hash, s_victim.raw_transaction)

        # Feed all three through detection engine
        for tx_hash, sender, gas_price, amount in [
            ...
            (sell_hash, bot2.address,   sell_gas, float(Web3.from_wei(AMT_BOT, "ether"))),
        ]:
